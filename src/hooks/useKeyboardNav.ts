	...
	const onKeyDown = useCallback(
		(event: React.KeyboardEvent<HTMLDivElement>) => {
			...
			if (event.key === "ArrowDown" || event.key === "ArrowUp") {
				...
				const currentIndex = activeElement?.dataset?.navIndex ? parseInt(activeElement.dataset.navIndex, 10) : -1;
				...
				const nextElement = document.querySelector<HTMLButtonElement>(`[data-nav-index="${nextIndex}"]`);
